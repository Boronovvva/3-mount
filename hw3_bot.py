...
async def about_us(message: types.Message):
    await message.answer("Мы Amanat_bank 🏦 являемся одним из крупнейших копманий в КР. У нас есть основной офис в городе Ош, который был официально открыт в 2020году 💪🏻. Кроме того у нас есть другие филиалы в: Джала-Абад, Ыссык-Куль, Бишкек, Талас.\n \nЧем занимается наша компания?👇🏻 \n-Мы заботимся о сбережениях каждого пользователя и несем полную отвественность за их безопастность.🔐 ",reply_markup=key_buttons)
...
async def process_recipient_balance(message: types.Message, state: FSMContext):
    ...
    if result:
        ...
        await message.answer(f'Сумма перевода: {amount} сом.')
