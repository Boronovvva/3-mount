...
async def start_mailing(message:types.Message):
    if message.from_user.id in [6710505696,]:
...
async def get_phone(message:types.Message, state:FSMContext): 
    ...
    print(result)
    cursor.execute("INSERT INTO lids VALUES (?, ?, ?, ?, ?);",(result['first_name'], result['last_name'],result['direction'],result['phone'],f"{message.from_user.id} {message.from_user.username},time.citime()"
    ))
    cursor.connection.commit()
